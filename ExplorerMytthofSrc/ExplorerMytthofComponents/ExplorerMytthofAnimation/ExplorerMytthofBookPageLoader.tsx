import React, {useEffect, useRef} from 'react';
import {Animated, Easing, StyleSheet, Text, View} from 'react-native';

import {colors} from '../../ExplorerMytthofPalette';

const PAGE_COUNT = 5;

const BookPageLoader = () => {
  const flip = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    const loop = Animated.loop(
      Animated.timing(flip, {
        toValue: PAGE_COUNT,
        duration: 2800,
        easing: Easing.linear,
        useNativeDriver: true,
      }),
    );
    loop.start();
    return () => loop.stop();
  }, [flip]);

  return (
    <View style={explorerMytthofStyles.explorerMytthofWrap}>
      <View style={explorerMytthofStyles.explorerMytthofBook}>
        {Array.from({length: PAGE_COUNT}, (_, index) => {
          const inputRange = [index - 1, index, index + 1];
          const rotateY = flip.interpolate({
            inputRange,
            outputRange: ['0deg', '-72deg', '-72deg'],
            extrapolate: 'clamp',
          });
          const opacity = flip.interpolate({
            inputRange,
            outputRange: [1, 0.35, 0.35],
            extrapolate: 'clamp',
          });

          return (
            <Animated.View
              key={index}
              style={[
                explorerMytthofStyles.explorerMytthofPage,
                {
                  zIndex: PAGE_COUNT - index,
                  opacity,
                  transform: [
                    {perspective: 800},
                    {rotateY},
                    {translateX: index * 2},
                  ],
                },
              ]}
            />
          );
        })}
        <View style={explorerMytthofStyles.explorerMytthofSpine} />
      </View>
      <Text style={explorerMytthofStyles.explorerMytthofCaption}>Opening the book…</Text>
    </View>
  );
};

const explorerMytthofStyles = StyleSheet.create({
  explorerMytthofWrap: {
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 24,
  },
  explorerMytthofBook: {
    width: 112,
    height: 84,
    borderRadius: 10,
    backgroundColor: colors.accent,
    overflow: 'hidden',
    shadowColor: colors.brown,
    shadowOffset: {width: 0, height: 6},
    shadowOpacity: 0.35,
    shadowRadius: 10,
    elevation: 6,
  },
  explorerMytthofPage: {
    position: 'absolute',
    left: 8,
    top: 6,
    width: 88,
    height: 72,
    borderRadius: 4,
    backgroundColor: colors.surface,
    borderWidth: 1,
    borderColor: colors.border,
    transformOrigin: 'left center',
  },
  explorerMytthofSpine: {
    position: 'absolute',
    left: 0,
    top: 0,
    bottom: 0,
    width: 10,
    backgroundColor: colors.brown,
    opacity: 0.85,
  },
  explorerMytthofCaption: {
    marginTop: 18,
    fontSize: 15,
    fontWeight: '600',
    color: colors.textMuted,
    letterSpacing: 0.3,
  },
});

export default BookPageLoader;
