...
import {Routes} from '../[navvyRoutes]/routes';
import type {MainTabParamList} from '../[navvyRoutes]/types';
import {AppLayout, ConfirmModal, StaggerItem} from '../cpnnts';
import {colors, gradients, gradientAxis} from '../anccpalEEtr';

import {resolveChronicle} from '../anccdatta/stories';
import type {BookmarkDisplay, BookmarkPhase} from '../annccTyppes';
import {
  loadShelfBookmarks,
  excerptFromTrail,
  discardBookmark,
...
const GlyphActionControl = ({
  ...
  <Pressable
    onPress={onActivate}
    style={({pressed}) => [
      glyphVariant === 'delete' ? styles.removeSurface : styles.emitSurface,
      pressed && styles.pressedState,
    ]}>
...
const SavedScreen = () => {
  const tabNavigation =
    ...
    >();
  const [screenPhase, setScreenPhase] = useState<BookmarkPhase>('shelf');
  const [shelfEntries, setShelfEntries] = useState<BookmarkDisplay[]>([]);
  ...
  const activeEntry = shelfEntries.find(e => e.entryKey === activeEntryKey);
          ...
          {shelfEntries.map((shelfEntry, index) => (
            <StaggerItem key={shelfEntry.entryKey} index={index}>
              <BookmarkListPanel
                ...
                onDiscard={() => setPendingDiscardKey(shelfEntry.entryKey)}
