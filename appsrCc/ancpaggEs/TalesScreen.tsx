...
} from '../anccdatta/stories';
import type {ChroniclePhase} from '../annccTyppes';
import {
  bookmarkExists,
  markChronicleConsumed,
  persistBookmark,
} from '../lnggkiiT';
import {StoryListCard} from '../cpnnts/tales';
import {
  AppLayout,
  FadeInView,
  GradientButton,
...
const TalesScreen = () => {
  const [screenPhase, setScreenPhase] = useState<ChroniclePhase>('catalog');
  const [activeEntryKey, setActiveEntryKey] = useState<string | null>(null);
  const [passageKey, setPassageKey] = useState('start');
  const [passageTrail, setPassageTrail] = useState<string[]>([]);
  const [trailPersisted, setTrailPersisted] = useState(false);

  const chronicle = useMemo(
    () => (activeEntryKey ? resolveChronicle(activeEntryKey) : undefined),
    [activeEntryKey],
  );

  const activePassage = chronicle?.passageMap[passageKey];

  const passageCloses = activePassage?.closesPassage === true;
  ...
  const openChronicle = useCallback(async (entryKey: string) => {
    const catalogItem = resolveChronicle(entryKey);
    if (!catalogItem) {
      return;
    }
    const alreadyBookmarked = await bookmarkExists(entryKey);
    setActiveEntryKey(entryKey);
    setPassageKey('start');
    setPassageTrail([catalogItem.openingPassage]);
  ...
  const selectBranch = useCallback(
    (onwardKey: string) => {
      ...
      const targetPassage = chronicle.passageMap[onwardKey];
      if (!targetPassage) {
        return;
      }
      const extendedTrail = [
  ...
  const persistPassageTrail = useCallback(async () => {
    if (!activeEntryKey || trailPersisted || !chronicle) {
      return;
    }
  ...
  }, [activeEntryKey, passageTrail, trailPersisted, chronicle]);
          ...
          return chroniclesGroupedByLocale.map(
            ({localeTag, groupedEntries}, regionIndex) => (
              <FadeInView
                ...
                <Text style={styles.regionHeading}>{localeTag}</Text>
                {groupedEntries.map(catalogItem => {
                  const item = (
                    <StaggerItem key={catalogItem.entryKey} index={cardIndex}>
                      <StoryListCard
                        story={catalogItem}
                        onOpen={() => openChronicle(catalogItem.entryKey)}
                      />
                    </StaggerItem>
                  );
                  cardIndex += 1;
  ...
  const showOpeningBanner = passageTrail.length === 1 && passageKey === 'start';
          ...
          <View style={styles.openingBanner}>
            <View style={styles.openingMediaFrame}>
              ...
              <View style={styles.openingTagChip}>
                <Text style={styles.listTagLabel}>{chronicle.localeTag}</Text>
              </View>
              <Text style={styles.openingTitle}>{chronicle.headline}</Text>
            </View>
          </View>
        )}
          ...
          <FadeInView style={styles.outcomeGroup} triggerKey="complete">
            ...
            <Pressable
              ...
              style={({pressed}) => [
                ...
                pressed && !trailPersisted && styles.pressedState,
              ]}>
              <LinearGradient
                ...
                <Text style={styles.branchLabel}>
                  {trailPersisted ? 'Your Story Saved' : 'Save This Version'}
                </Text>
              </LinearGradient>
            </Pressable>
          </FadeInView>
        ) : (
          <FadeInView style={styles.branchOptions} triggerKey={passageKey}>
            ...
            {activePassage.branchOptions?.map((branchPick, choiceIndex) => (
              <StaggerItem key={branchPick.onwardKey} index={choiceIndex}>
                <GradientButton
                  label={branchPick.optionCaption}
                  tall={branchPick.optionCaption.length > 42}
                  onPress={() => selectBranch(branchPick.onwardKey)}
                />
              </StaggerItem>
            ))}
          </FadeInView>
